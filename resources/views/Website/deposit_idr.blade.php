...
<body class="index-page">
        ...
        <form action="{{ route('midtrans') }}" method="post">
                ...
                @foreach ($daftarSaldos as $saldo)
                    <div class="col-md-6 col-lg-4 col-saldo mt-4">
                        <label class="saldo mb-3" id="saldo-{{ $saldo->id }}"
                            onclick="selectSaldo('{{ $saldo->id }}', '{{ $saldo->idr }}')">
                            <input type="radio" name="selectedSaldo" value="{{ $saldo->idr }}">
                            <h6 class="text-saldo text-center p-2">IDR {{ number_format($saldo->idr) }}</h6>
                        </label>
                    </div>
                @endforeach
            </div>
            <div class="mt-6">
                <button class="btn btn-secondary float-right" type="submit"
                    style="font-style: italic; font-weight:900;">Pay now <img
                        src="https://cdn-icons-png.flaticon.com/512/9181/9181081.png" alt="" srcset=""
    ...
    <script>
        function selectSaldo(id, idr) {
            ...
            $("input[name=selectedSaldo][value='" + idr + "']").prop('checked', true);
